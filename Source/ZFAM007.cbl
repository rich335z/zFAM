      ...
      *****************************************************************
       ...
       01  SELECT-LENGTH          PIC S9(08) COMP VALUE 06.
      ...
      *****************************************************************
      * Issue READNEXT for KEY store record.                          *
      *****************************************************************
       3200-READ-KEY.
           ...
           IF  SELECT-TYPE(1:4)     EQUAL 'text'
           AND FK-OBJECT            EQUAL 'text'
               MOVE 'Y' TO SELECT-MATCH
               ADD 1    TO ROWS-COUNT.

           IF  PROCESS-COMPLETE EQUAL 'N'
           IF  SELECT-TYPE(1:4) NOT EQUAL 'text'
           AND FK-OBJECT        NOT EQUAL 'text'
               MOVE 'Y' TO SELECT-MATCH
               ADD 1    TO ROWS-COUNT.
      ...
      *****************************************************************
       3300-READ-FILE.
           ...
           PERFORM 3400-SEGMENTS      THRU 3400-EXIT
               WITH TEST AFTER
               UNTIL SEGMENTS-COMPLETE   EQUAL  'Y'
               OR    SEGMENTS-SUCCESSFUL EQUAL  'N'.
           ...
           IF  SEGMENTS-SUCCESSFUL = 'Y'
               IF  GET-CA-DELIM   NOT = SPACES
               AND GET-CA-DELIM   NOT = LOW-VALUES
                   IF  MESSAGE-COUNT  = ZEROES
      ...
      * record, set the BUFFER-FULL condition and return HTTP status  *
      * code 206.                                                     *
      *****************************************************************
       3420-CHECK-LENGTH.
